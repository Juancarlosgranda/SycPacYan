...
class DetalleSalidaController extends Controller {
	...
	*/
		...
		else
		{
			...
			foreach ($detalles as $detalle) {
				...
				if($producto->prod_exo=='NO')
				{
					...
					$igvtot=$igvtot+round((($detalle->dcomp_cant * $detalle->dcomp_prec)*0.18/1.18),2);
					$subtot=$subtot+round((($detalle->dcomp_cant * $detalle->dcomp_prec)/1.18),2);
				}
				else
				{
			...
			$comprobante=Comprobante::find($comp_id);			
			$comprobante->comp_subt=$totalsigv+$subtot;
			$comprobante->comp_igv=$igvtot;
	...
	public function getGenerartxt(Request $request)
	{
		...
		return view('detallesalida.txt',['detallecomprobantes'=> $detallecomprobantes,'comprobante'=> $comprobante, 'nro_correlativo'=>$nro_correlativo]);
	}
