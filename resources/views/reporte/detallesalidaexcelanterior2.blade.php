		...
		<font size='6' color='#084B8A'><center>REPORTE DE VENTAS</center></font>
			...
			@if(sizeof($detallecomprobantes)>0)
				...
				@foreach ($detallecomprobantes as $detallecomprobante)
					...
					<tr>
						@if($flag)
						<td rowspan="{{$nro_det}}" style="vertical-align: middle; text-align:left;"><strong>{{$detallecomprobante->comprobante->comp_nro}}</strong></td>
						<td rowspan="{{$nro_det}}" style="vertical-align: middle; text-align:left;"><strong>{{substr($detallecomprobante->comprobante->tipocomprobante->tcomp_desc,0,3)}}</strong></td>
						<td rowspan="{{$nro_det}}" style="vertical-align: middle; text-align:left;"><strong>{{$detallecomprobante->comprobante->entidad->ent_ruc}}</strong></td>
						<td rowspan="{{$nro_det}}" style="vertical-align: middle; text-align:left;"><strong>{{$detallecomprobante->comprobante->entidad->ent_rz}}</strong></td>
						<td rowspan="{{$nro_det}}" style="vertical-align: middle; text-align:left;"><strong>{{date('d/m/Y', strtotime($detallecomprobante->comprobante->comp_fecha))}}</strong></td>

						<?php
							if($detallecomprobante->comprobante->comp_moneda=='SOLES'){?>
								<td rowspan="{{$nro_det}}" style="mso-number-format:'#,##0.00;-#,##0.00';vertical-align: middle; text-align:right;"><strong>{{number_format($detallecomprobante->comprobante->comp_subt,2,'.',',')}}</strong></td>
								<td rowspan="{{$nro_det}}" style="mso-number-format:'#,##0.00;-#,##0.00';vertical-align: middle; text-align:right;"><strong>{{number_format($detallecomprobante->comprobante->comp_igv,2,'.',',')}}</strong></td>
								<td rowspan="{{$nro_det}}" style="mso-number-format:'#,##0.00;-#,##0.00';vertical-align: middle; text-align:right;"><strong>{{number_format($detallecomprobante->comprobante->comp_tot,2,'.',',')}}</strong></td>
							<?php 
							}else { ?>
								<td rowspan="{{$nro_det}}" style="mso-number-format:'#,##0.00';vertical-align: middle; text-align:right;"><strong>{{number_format($detallecomprobante->comprobante->comp_subt,2,'.',',')}}</strong></td>
								<td rowspan="{{$nro_det}}" style="mso-number-format:'#,##0.00';vertical-align: middle; text-align:right;"><strong>{{number_format($detallecomprobante->comprobante->comp_igv,2,'.',',')}}</strong></td>
								<td rowspan="{{$nro_det}}" style="mso-number-format:'#,##0.00';vertical-align: middle; text-align:right;"><strong>{{number_format($detallecomprobante->comprobante->comp_tot,2,'.',',')}}</strong></td>
						<?php }?>
						
						<td rowspan="{{$nro_det}}" style="mso-number-format:'#,##0.00';vertical-align: middle; text-align:right;"><strong>{{number_format($detallecomprobante->comprobante->comp_saldo,2,'.',',')}}</strong></td>
						<td rowspan="{{$nro_det}}" style="vertical-align: middle; text-align:left;"><strong>{{$detallecomprobante->comprobante->comp_moneda}}</strong></td>
